import { useState } from 'react';
import styled from 'styled-components';
import { useMyContext } from '../context/context';

const CardDisplay = styled.div`position: relative;`;

const CardStructure = styled.div`
	color: white;
	aspect-ratio: 3/4;
	width: 350px;
	background-color: transparent;
	border-image: url("data:image/png;base64,")
		28 / 28px / 0 round;
	border-width: 8px;
	border-style: solid;
	position: absolute;
	z-index: 6;
`;

const CardAtributes = styled.div`
	color: white;
	aspect-ratio: 3/4;
	width: calc(350px + 16px);

	background-color: transparent;
	position: relative;
	z-index: 8;
	display: grid;
`;

const CardStructureInside = styled.div`
	color: white;

	height: 100%;

	display: grid;
	grid-template-rows: 50% 12% 38%;
	border-image: url("data:image/png;base64,")
		28 / 28px / 0 round;
	border-width: 8px;
	border-style: solid;
	/* z-index: 4; */
	position: relative;
`;

const CardImage = styled.div`
	/* position: absolute; */
	/* z-index: 2; */
	img {
		width: 100%;
		height: 100%;
		object-fit: cover;
	}
`;

const CardTitle = styled.div`
	text-align: center;
	font-family: 'Amiga Forever', sans-serif;
	background-color: white;
	color: #1b1a1a;
	width: calc(100% + 8px);
	margin-left: -8px;
	font-size: 0.8rem;
	margin-top: auto;
	margin-bottom: auto;
	padding: 0.4rem;
`;

const Description = styled.div`
	background-color: #1d1a1a !important;
	border-radius: 0px 0px 15px 15px;
	font-family: 'Amiga Forever', sans-serif !important;
	display: grid;
	text-align: left;
	font-size: 0.6rem;
	padding: 1rem;

	p {
		background-color: #1d1a1a !important;
		font-size: 0.6rem !important;
	}
`;

function Card() {
	const { data } = useMyContext();
	console.log('Data ->', data);
	let { cards_records, page, total_pages } = data;
	let actualCard = cards_records[0];
	const {
		ATK,
		CATEGORY,
		CLASS,
		DEF,
		DEFAULT_IMAGE,
		DEFAULT_PIXEL_IMAGE,
		DESCRIPTION,
		EFFECT,
		IMAGES,
		NAME,
		VALID,
		VISUAL_DESCRIPTION,
        id
	} = actualCard;

	return (
		<CardDisplay>
			{' '}
			<CardStructure>
				<CardStructureInside>
					<CardImage>
						<img
							src={`https://raw.githubusercontent.com/ermsharo/cards_images_processing/main/image_%5B${id}%5D/0.jpeg`}
						/>
					</CardImage>
					<CardTitle>{NAME}</CardTitle>
					<Description>
						<div className="nes-container with-title">
							<p className="title">{CLASS}</p>
							<p>{DESCRIPTION}</p>
						</div>
					</Description>
				</CardStructureInside>
			</CardStructure>
			<CardAtributes />
		</CardDisplay>
	);
}

export default Card;
